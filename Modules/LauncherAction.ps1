...
function Start-SpotifyAutoplay {
    ...
    Join-Path `
    $AppRoot `
    "Programs\SpotifyPlay.vbs"

    if(
        Test-Path $spotifyScript
    )
    {

        Start-Process `
        -FilePath "$env:WINDIR\System32\wscript.exe" `
        -ArgumentList "`"$spotifyScript`"" `
        -WindowStyle Hidden
...
function Invoke-ProgramLaunch {
    ...
    )
    {
        ...
        $launchIdentity =
        (([string]$program.Path).Trim().ToLowerInvariant() + "|" + ([string]$program.Args).Trim())
        ...
        catch
        {
            ...
            -Exception $_ `
            -Context "Failed to launch '$name' from '$($program.Path)'."
    ...
    Refresh-StatusDelayed `
    -Programs $Programs `
    -Statuses $Statuses `
    -ProgramsToUpdate $updated

    Start-SpotifyAutoplay `
    -AppRoot $AppRoot
...
function Invoke-ProgramTermination {
    ...
    )
    {
        ...
        -WindowStyle Hidden `
        -Verb RunAs `
        -ArgumentList "-WindowStyle Hidden -Command `$p=[Text.Encoding]::Unicode.GetString([Convert]::FromBase64String('$encoded')); `$p.Split(',') | ForEach-Object { Stop-Process -Name `$_.Replace('.exe','') -Force -ErrorAction SilentlyContinue }"
...
$Form.Add_SizeChanged({
    ...
    if($TerminateSelected)
    {
        ...
        $TerminateSelected.Left =
        $leftMargin +
        $LaunchSelected.Width +
        $buttonSpacing

        $TerminateSelected.Top =
        $LaunchSelected.Top
...
$launchAll.Add_Click({
    ...
    if(
        $terminateButton -and
        -not $terminateButton.IsDisposed
    )
    {

        $terminateButton.Enabled =
        $false
...
$terminateAll =
New-TerminateAllButton

$launchAllState.TerminateButton =
$terminateAll

$terminateAll.Add_Click({

    Invoke-ProgramTermination `
    -Programs $Programs `
    -Statuses $Statuses `
    -ProgramNames @(
        $Programs.Keys
...
Add-LauncherButtonResizeHandler `
-Form $Form `
-LaunchAll $launchAll `
-TerminateAll $terminateAll `
-LaunchSelected $launchSelected `
-TerminateSelected $action

Set-LauncherButtonInitialPosition `
-Form $Form `
-LaunchAll $launchAll `
-TerminateAll $terminateAll `
-LaunchSelected $launchSelected `
