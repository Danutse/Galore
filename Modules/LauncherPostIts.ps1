...
if(
    -not ("GalorePostItDpi.Native" -as [type])
)
{
    Add-Type @"
using System;
...
function Enter-GalorePostItDpiContext {
    ...
    try
    {
        ...
        return [GalorePostItDpi.Native]::SetThreadDpiAwarenessContext(
            [IntPtr](-4)
        )
...
function Exit-GalorePostItDpiContext {
    ...
    if(
        $PreviousContext -eq [IntPtr]::Zero
    )
    {
        return
    }
    ...
    try
    {
        ...
        [GalorePostItDpi.Native]::SetThreadDpiAwarenessContext(
            $PreviousContext
        ) | Out-Null
...
function Resolve-GalorePostItLocation {
    ...
    foreach($screen in [System.Windows.Forms.Screen]::AllScreens)
    {
        ...
        [int64]$visibleAreaSize =
        [int64]$visibleArea.Width *
    ...
    if($null -ne $bestScreen)
    {
        ...
        return [System.Drawing.Point]::new(
            [Math]::Min(
                [Math]::Max($X, $workingArea.Left),
                $maximumX
            ),
            [Math]::Min(
                [Math]::Max($Y, $workingArea.Top),
                $maximumY
            )
        )
    }
    ...
    $offset =
    ($script:GalorePostIts.Count % 6) * 26

    return [System.Drawing.Point]::new(
        [int]($workingArea.Left + 60 + $offset),
        [int]($workingArea.Top + 60 + $offset)
    )
...
function Get-GalorePostItStates {
    ...
    )
    {
        ...
        if(
            $null -eq $noteContext -or
            -not $noteContext.KeepInPersistence -or
            $null -eq $noteContext.TextBox -or
            $noteContext.TextBox.IsDisposed
        )
        {
            continue
        }
...
function Save-GalorePostIts {
    ...
    $stateFolder =
    Split-Path `
    -Parent $script:GalorePostItStorePath

    $stateFileName =
    Split-Path `
    -Leaf $script:GalorePostItStorePath

    $temporaryPath =
    Join-Path `
    $stateFolder `
    ".${stateFileName}.$([guid]::NewGuid().ToString('N')).tmp"
    ...
    try
    {
        ...
        foreach(
            $savedState in @(Get-GalorePostItStates)
        )
        {
            [void]$noteStates.Add(
                $savedState
        ...
        if(
            Test-Path `
            -LiteralPath $script:GalorePostItStorePath `
            -PathType Leaf
        )
        {
            $backupPath =
            Join-Path `
            $stateFolder `
            ".${stateFileName}.$([guid]::NewGuid().ToString('N')).previous"
    ...
    catch
    {
        ...
        -Exception $_ `
        -Context "Failed to save Post-it state."
    }
    finally
    {
        if(
...
function Read-GalorePostItStates {
    ...
    try
    {
        ...
        if(
            $null -eq $document.PSObject.Properties["Notes"]
        )
        {
        ...
        )
        {
            ...
            if(
                $null -eq $savedNote -or
                $null -eq $savedNote.PSObject.Properties["Text"] -or
                -not [int]::TryParse([string]$savedNote.X, [ref]$x) -or
                -not [int]::TryParse([string]$savedNote.Y, [ref]$y)
            )
            {
                continue
            }
            ...
            if(
                $null -ne $savedNote.PSObject.Properties["Id"] -and
                -not [string]::IsNullOrWhiteSpace([string]$savedNote.Id)
            )
            {
                $id =
                [string]$savedNote.Id
            ...
            if(
                $null -ne $savedNote.PSObject.Properties["Checked"]
            )
            {
                $checked =
                [bool]$savedNote.Checked
            }
    ...
    catch
    {
        Write-LauncherDiagnostic `
        -Exception $_ `
        -Context "Failed to load Post-it state."
...
function Move-GalorePostItDrag {
    ...
    [System.Drawing.Point]::new(
        [int]([int]($cursor.X - $note.Tag.DragOffset.X)),
        [int]([int]($cursor.Y - $note.Tag.DragOffset.Y))
    )
...
function Set-GalorePostItCompletionStyle {
    ...
    )
    {
        ...
        New-Object System.Drawing.Font(
            $Editor.Font,
            (
                [System.Drawing.FontStyle]::Strikeout -bor
...
function New-GalorePostItIconBitmap {
    ...
    $graphics =
    [System.Drawing.Graphics]::FromImage($icon)
...
function Set-GalorePostItRegion {
    ...
    $Form.Region =
    New-Object System.Drawing.Region($path)
...
function New-GalorePostIt {
    ...
    $previousDpiContext =
    Enter-GalorePostItDpiContext

    try
    {

        return New-GalorePostItWindow `
        -State $State
    ...
    finally
    {
        ...
        Exit-GalorePostItDpiContext `
        -PreviousContext $previousDpiContext
...
function New-GalorePostItWindow {
    ...
    Get-GalorePostItImage `
    -Name "PostitBackground.png"

    $note.BackgroundImageLayout =
    [System.Windows.Forms.ImageLayout]::None

    Set-GalorePostItRegion `
    -Form $note `
    -Bitmap $note.BackgroundImage

    Set-GalorePostItFixedLayout `
    -Form $note `
    -X $State.X `
    -Y $State.Y
    ...
    $editor.Add_TextChanged({
        ...
        [System.Windows.Forms.TextRenderer]::MeasureText(
            $sender.Text,
            $sender.Font,
            (New-Object System.Drawing.Size($sender.ClientSize.Width, 0)),
            (
        ...
        if(
            $measuredSize.Height -gt $sender.ClientSize.Height -and
            $sender.CanUndo
        )
        {
            $sender.Undo()
    ...
    $closeBounds =
    New-Object System.Drawing.Rectangle(241, 7, 12, 12)

    $closeIcon =
    New-GalorePostItIconBitmap `
    -Background $note.BackgroundImage `
    -Bounds $closeBounds `
    -DrawOverlay {
        param(
    $graphics,
    ...
    $uncheckedBounds =
    New-Object System.Drawing.Rectangle(14, 7, 18, 18)
    ...
    [int]$height
        ...
        $boxPen =
        New-Object System.Drawing.Pen(
            [System.Drawing.Color]::FromArgb(110, 210, 140),
            1.5
        )
        ...
        $graphics.DrawLines(
            $checkPen,
            @(
                (New-Object System.Drawing.Point(4, [int]($height / 2))),
                (New-Object System.Drawing.Point([int]([int]($width / 2) - 1), ([int]$height - 5))),
                (New-Object System.Drawing.Point(([int]$width - 4), 4))
            )
        )
    ...
    $completeBox.Image =
    if([bool]$State.Checked) { $checkedIcon } else { $uncheckedIcon }

    $dragHandle =
    New-Object System.Windows.Forms.Panel

    $dragHandle.BackColor =
    ...
    $dragHandle.Add_MouseUp({
        param($sender, $e)
        Stop-GalorePostItDrag `
        -Sender $sender
    })

    Set-GalorePostItCompletionStyle `
    -Editor $editor `
    -Checked ([bool]$State.Checked)
    ...
    $completeBox.Add_Click({
        ...
        $note.Tag.Checked =
        -not $note.Tag.Checked

        $sender.Image =
        if($note.Tag.Checked) { $checkedIcon } else { $uncheckedIcon }

        Set-GalorePostItCompletionStyle `
        -Editor $editor `
        -Checked ([bool]$note.Tag.Checked)
    ...
    $closeButton.Add_Click({
        ...
        $note.Tag.KeepInPersistence =
        $false

        if(
            $null -ne $script:GalorePostIts
        )
        {
    ...
    $note.Add_FormClosed({
        param($sender, $e)
        if(
            $null -ne $script:GalorePostIts -and
            $null -ne $sender
        )
        {
            [void]$script:GalorePostIts.Remove(
                $sender
            )
        }
...
function Restore-GalorePostIts {
    ...
    )
    {
        ...
        catch
        {
            Write-LauncherDiagnostic `
            -Exception $_ `
            -Context "Failed to restore a Post-it."
        }
    }
...
function Initialize-GalorePostIts {
    ...
    $legacyPostItStorePath =
    Join-Path `
    $AppRoot `
    "postits.json"

    if(
        -not (Test-Path -LiteralPath $script:GalorePostItStorePath -PathType Leaf) -and
        (Test-Path -LiteralPath $legacyPostItStorePath -PathType Leaf)
    )
    {
        try
        {
        ...
        catch
        {
            Write-LauncherDiagnostic `
            -Exception $_ `
            -Context "Failed to move legacy Post-it state into Settings."
        }
    }
...
function Stop-GalorePostItResources {
    ...
    if(
        $script:GalorePostItShuttingDown
    )
    {
        return
    }
    ...
    foreach($note in @($script:GalorePostIts))
    {
        if(
            $null -ne $note -and
            -not $note.IsDisposed -and
            $null -ne $note.Tag -and
            $note.Tag.Checked
        )
        {
            $note.Tag.KeepInPersistence =
