...
function Apply-GaloreProgramOverrides {
    ...
    foreach($programName in @($overrides.Keys))
    {
        ...
        if(
            -not $Programs.Contains($programName)
        )
        {
        ...
        )
        {
            ...
            -Level "WARNING" `
            -Component "Programs" `
            -Message "Saved override for '$programName' was ignored because its executable is unavailable."
        ...
        $Programs[$programName]["Path"] =
        [string]$override.Path

        $Programs[$programName]["Args"] =
        ""

        $Programs[$programName]["StatusProcess"] =
        $processName

        $Programs[$programName]["WindowProcess"] =
        $processName

        $Programs[$programName]["DisplayName"] =
...
function Show-GaloreProgramCustomization {
    ...
    $fileDialog.Title =
    "Choose the executable for $ProgramName"
    ...
    $currentPath =
    [string]$Programs[$ProgramName]["Path"]
    ...
    catch
    {
        ...
        -Exception $_ `
        -Context "Failed to customize launcher program '$ProgramName'."

        [System.Windows.Forms.MessageBox]::Show(
            "Galore could not save the selected program. See Diagnostics.log for details.",
            "Galore Launcher"
        ) | Out-Null
...
if(
    $name -eq "Browser" -and
    $Programs[$name].Contains("BrowserDisplayName")
)
{
...
elseif(
    $Programs[$name].Contains("DisplayName") -and
...
Register-ProgramNameToggleEvent `
$nameLabel
...
$box.Add_CheckedChanged({
    ...
    if(
        $null -ne $this.Tag -and
        $this.Tag -is [System.Windows.Forms.Label]
    )
    {
...
$box.Add_MouseUp({
    ...
    )
    {
        ...
        if($programName -eq "Browser")
        {
            ...
            Show-GaloreBrowserSelector `
            -Programs $Programs `
            -BrowserLabel $this.Tag `
            -AppRoot $AppRoot
        ...
        else
        {
            ...
            Show-GaloreProgramCustomization `
            -Programs $Programs `
            -ProgramName $programName `
            -ProgramLabel $this.Tag
...
function Update-ProgramStatus {
    ...
    )
    {
        ...
        $process =
        $Programs[$name]["StatusProcess"]

        $isRunning =
        [bool](
            Get-ProgramStatus $process
        )

        Update-GaloreApplicationMaintenanceState `
...
function Refresh-StatusDelayed {
    ...
    if(
        $null -eq $script:StatusRefreshTimers
    )
    {

        $script:StatusRefreshTimers =
        New-Object System.Collections.ArrayList

    }

    $script:StatusRefreshTimers.Add(
        $refreshTimer
    ) | Out-Null
    ...
    $refreshTimer.Add_Tick({
        ...
        try
        {
            ...
            -ProgramsToUpdate $state.ProgramsToUpdate
        ...
        finally
        {
            ...
            )
            {
                ...
                $script:StatusRefreshTimers.Remove(
                    $timer
                ) | Out-Null
...
function Stop-ProgramStatusResources {
    ...
    if(
        $script:ProgramStatusTimer
    )
    {

        $script:ProgramStatusTimer.Stop()

        $script:ProgramStatusTimer.Tag =
        $null
