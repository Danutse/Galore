...
$script:GaloreActivityLogFile =
Join-Path `
$LogFolder `
"GaloreLauncher.log"

$LegacyGaloreActivityLogFile =
Join-Path `
$LogFolder `
"Galore.log"
...
if(
    -not (
        Test-Path `
        -LiteralPath $script:GaloreActivityLogFile `
        -PathType Leaf
    ) -and
    (
        Test-Path `
        -LiteralPath $LegacyGaloreActivityLogFile `
        -PathType Leaf
    )
)
{
    ...
    try
    {
        ...
        Move-Item `
        -LiteralPath $LegacyGaloreActivityLogFile `
        -Destination $script:GaloreActivityLogFile `
        -ErrorAction Stop
...
"@
        try
        {
            ...
            Add-Content `
            -LiteralPath $script:DiagnosticLogFile `
            -Value $detailEntry `
            -Encoding UTF8 `
            -ErrorAction Stop
