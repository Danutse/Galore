...
function Initialize-LauncherSettings {
    ...
    if(
        -not (Test-Path -LiteralPath $script:SettingsFile -PathType Leaf) -and
        (Test-Path -LiteralPath $legacySettingsFile -PathType Leaf)
    )
    {
...
function ConvertTo-ValidatedLauncherSettings {
    ...
    )
    {
        ...
        if(
            $null -eq $Settings.PSObject.Properties[$propertyName]
        )
        {
    ...
    if(
        $null -ne $Settings.PSObject.Properties["BrowserId"] -and
        $null -ne $Settings.BrowserId
    )
    {
    ...
    if(
        $null -ne $Settings.PSObject.Properties["ProgramOverrides"] -and
        $null -ne $Settings.ProgramOverrides
    )
    {

        $rawOverrides =
        $Settings.ProgramOverrides
        ...
        else
        {
            ...
            $overrideNames =
            @($rawOverrides.PSObject.Properties.Name)
        ...
        foreach($overrideName in $overrideNames)
        {
            ...
            if(
                $null -eq $override -or
                $null -eq $override.PSObject.Properties["Path"] -or
                $override.Path -isnot [string] -or
                [string]::IsNullOrWhiteSpace($override.Path)
            )
            {
            ...
            if(
                $null -ne $override.PSObject.Properties["DisplayName"] -and
                $null -ne $override.DisplayName
            )
            {
            ...
            $programOverrides[[string]$overrideName] =
            [pscustomobject]@{
...
function Resolve-LauncherWindowPlacement {
    ...
    )
    {
        ...
        if(
            $visibleRectangle.Width -ge 120 -and
            $visibleRectangle.Height -ge 30
        )
        {
    ...
    $workingArea.Left +
    [math]::Floor(
        ($workingArea.Width - $visibleWidth) / 2
    )

    $visibleY =
    $workingArea.Top +
    [math]::Floor(
        ($workingArea.Height - $visibleHeight) / 2
    )

    if(
        -not $script:WindowPlacementCorrectionLogged
    )
    {

        $placementError =
        New-Object System.InvalidOperationException(
            "Saved window position ($($Settings.X), $($Settings.Y)) is not visible on any connected screen."
        )

        Write-LauncherDiagnostic `
        -Exception $placementError `
        -Context "Launcher window placement was reset to the primary screen."
...
function Read-LauncherSettingsFile {
    ...
    return ConvertTo-ValidatedLauncherSettings `
    -Settings $settings
...
function Load-WindowSettings {
    ...
    if(
        -not (
            Test-Path `
            -LiteralPath $script:SettingsFile `
            -PathType Leaf
        )
    )
    {
    ...
    try
    {
        ...
        $settings =
        Read-LauncherSettingsFile `
        -Path $script:SettingsFile

        return Resolve-LauncherWindowPlacement `
        -Settings $settings
    ...
    catch
    {
        ...
        Write-LauncherDiagnostic `
        -Exception $_ `
        -Context "Failed to read launcher settings from '$script:SettingsFile'."
...
function Get-LauncherSaveBounds {
    ...
    if(
        $null -eq $windowBounds -or
        $windowBounds.Width -lt 1 -or
        $windowBounds.Height -lt 1
    )
    {

        $windowBounds =
        New-Object System.Drawing.Rectangle(
            $script:LauncherLocation.X,
            $script:LauncherLocation.Y,
            $script:LauncherSize.Width,
            $script:LauncherSize.Height
        )
...
function Save-WindowSettings {
    ...
    -Settings ([pscustomobject]@{
        ...
        ProgramOverrides =
        Get-LauncherProgramOverrides

    })

    $settings =
    Resolve-LauncherWindowPlacement `
    -Settings $settings
    ...
    catch
    {
        ...
        Write-LauncherDiagnostic `
        -Exception $_ `
        -Context "Failed to save launcher settings atomically to '$script:SettingsFile'."
    ...
    finally
    {
        ...
        if(
            Test-Path `
            -LiteralPath $temporarySettingsFile `
            -PathType Leaf
        )
        {
        ...
        if(
            $discardedSettingsFile -and
            (
                Test-Path `
                -LiteralPath $discardedSettingsFile `
                -PathType Leaf
            )
        )
        {
...
function Get-LauncherProgramOverrides {
    ...
    catch
    {
        ...
        Write-LauncherDiagnostic `
        -Exception $_ `
        -Context "Failed to read saved program overrides from '$script:SettingsFile'."
...
function Save-LauncherProgramOverrides {
    ...
    catch
    {
        ...
        Write-LauncherDiagnostic `
        -Exception $_ `
        -Context "Failed to save program overrides atomically to '$script:SettingsFile'."
