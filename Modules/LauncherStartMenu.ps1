...
$GaloreModuleManifest = [ordered]@{
    ...
    RequiresFiles = @(
        "Resources\exe.png", "Resources\folder.png",
        "Resources\image.png", "Resources\shortcut.png"
    )
    ProvidesTypes = @()
}
...
function Clear-StartSearchResults {
    ...
    if(
        $null -eq $script:StartSearchResults -or
        $script:StartSearchResults.IsDisposed
    )
    {
    ...
    foreach(
        $resultControl in
        @($script:StartSearchResults.Controls)
    )
    {
...
function ConvertTo-SearchResultImage {
    ...
    try
    {
        ...
        $scale =
        [Math]::Min(
            (28.0 / [double]$SourceImage.Width),
            (28.0 / [double]$SourceImage.Height)
        )
...
function Get-SearchResultImage {
    ...
    try
    {
        ...
        return ConvertTo-SearchResultImage `
...
function Get-StartSearchPanelTop {
    ...
    if(
        $script:StartMenuForm -and
        -not $script:StartMenuForm.IsDisposed
    )
    {

        $programTitleBar =
        $script:StartMenuForm.Controls["ProgramTitleBar"]

        if(
            $programTitleBar -and
            -not $programTitleBar.IsDisposed
        )
        {
    ...
    if(
        $script:StartSearchPanel -and
        -not $script:StartSearchPanel.IsDisposed -and
        $script:StartSearchPanel.Top -gt 0
    )
    {
...
function Set-WindowsSearchToggleBounds {
    ...
    if(
        $null -eq $script:WindowsButton -or
        $script:WindowsButton.IsDisposed
    )
    {
    ...
    if($Compact)
    {
        ...
        if(
            $null -eq $script:WindowsButtonNormalBounds
        )
        {

            $script:WindowsButtonNormalBounds =
            New-Object System.Drawing.Rectangle(
                $script:WindowsButton.Left,
                $script:WindowsButton.Top,
                $script:WindowsButton.Width,
                $script:WindowsButton.Height
            )

        }

        $panelTop =
        Get-StartSearchPanelTop
    ...
    if(
        $null -ne $script:WindowsButtonNormalBounds
    )
    {
...
function Stop-StartSearchPanelAnimation {
    ...
    if(
        $null -eq $script:SearchPanelAnimationTimer
    )
    {
    ...
    $script:SearchPanelAnimationTimer.Tag =
    $null
...
function Set-StartSearchPanelBounds {
    ...
    param(
        [bool]$ShowPanel =
        $script:SearchPanelTargetVisible
    )

    if(
        $null -eq $script:StartMenuForm -or
        $script:StartMenuForm.IsDisposed -or
        $null -eq $script:StartSearchPanel -or
        $script:StartSearchPanel.IsDisposed
    )
    {
    ...
    $script:StartSearchPanel.Location =
    New-Object System.Drawing.Point(
        $panelLeft,
        $panelTop
    )

    $script:StartSearchPanel.Height =
    [Math]::Max(
        0,
        (
            $script:StartMenuForm.ClientSize.Height -
            $panelTop
        )
    )
...
function Start-SearchPanelSlideAnimation {
    ...
    if(
        $DurationMilliseconds -le 0 -or
        $panel.Left -eq $targetLeft
    )
    {
    ...
    $animationTimer.Add_Tick({
        ...
        if(
            $null -eq $state -or
            $null -eq $state.Panel -or
            $state.Panel.IsDisposed -or
        ...
        )
        {
            ...
            if(
                $script:SearchPanelAnimationTimer -eq $timer
            )
            {
        ...
        [Math]::Max(
            0,
            (
                $state.Form.ClientSize.Height -
                $panelTop
            )
        )

        Set-WindowsSearchToggleBounds `
        -PanelLeft $state.Panel.Left `
        -Compact $true
        ...
        if($state.ShowPanel)
        {
            ...
            if(
                $script:StartMenuForm -and
                -not $script:StartMenuForm.IsDisposed -and
                $script:StartSearchBox -and
                -not $script:StartSearchBox.IsDisposed
            )
            {
...
function Show-StartSearchWindowAnimated {
    ...
    Start-SearchPanelSlideAnimation `
    -ShowPanel $true `
...
function Show-StartSearchWindow {
    ...
    if(
        $script:StartSearchPanel -and
        -not $script:StartSearchPanel.IsDisposed
    )
    {
    ...
    $script:SearchDelayTimer =
    New-Object System.Windows.Forms.Timer

    $script:SearchDelayTimer.Interval =
    250
    ...
    $script:StartSearchBox.Add_TextChanged({
        ...
        if(
            $null -eq $script:SearchDelayTimer
        )
        {
        ...
        $script:PendingSearch =
        $script:StartSearchBox.Text
    ...
    $script:SearchDelayTimer.Add_Tick({
        ...
        )
        {
            ...
            $resultButton.ImageAlign =
            [System.Drawing.ContentAlignment]::MiddleLeft

            $resultButton.TextImageRelation =
            [System.Windows.Forms.TextImageRelation]::ImageBeforeText

            $resultButton.TextAlign =
            [System.Drawing.ContentAlignment]::MiddleLeft
            ...
            $resultButton.FlatStyle =
            [System.Windows.Forms.FlatStyle]::Flat

            $resultButton.FlatAppearance.BorderSize =
            0

            $resultButton.BackColor =
            ...
            $script:StartSearchResults.Controls.Add(
                $resultButton
            )
...
function Initialize-StartMenu {
    ...
    )
    {
        ...
        $script:StartMenuForm.Add_Resize({
            ...
            )
            {
                ...
                Set-StartSearchPanelBounds `
                -ShowPanel $script:SearchPanelTargetVisible

                $script:StartSearchPanel.Visible =
                $script:SearchPanelTargetVisible

                Set-WindowsSearchToggleBounds `
                -PanelLeft $script:StartSearchPanel.Left `
                -Compact $script:SearchPanelTargetVisible
    ...
    $WindowsUI =
    New-WindowsStartButton

    $script:WindowsButton =
    $WindowsUI.Button

    $script:WindowsButton.Name =
    "WindowsSearchToggle"

    $script:WindowsButton.AccessibleName =
    "WindowsSearchToggle"
