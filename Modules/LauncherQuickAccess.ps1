...
function Get-GaloreQuickAccessItems {
    ...
    try
    {
        $document = Get-Content -LiteralPath $script:GaloreQuickAccessPath -Raw -ErrorAction Stop |
        ...
        foreach($item in @($document.Items))
        {
            if(
                ...
                (Test-Path -LiteralPath ([string]$item.Path))
            )
            {
...
function Remove-GaloreQuickAccessItem {
    ...
    $removeItem =
    @($script:GaloreQuickAccessItems | Where-Object { $_.Path -eq $Path }) |
...
function Add-GaloreQuickAccessDroppedItems {
    ...
    try
    {
        ...
        if(
            $null -eq $Data -or
            -not $Data.GetDataPresent([System.Windows.Forms.DataFormats]::FileDrop)
        )
        {
            return
        }
        ...
        foreach($path in @($Data.GetData([System.Windows.Forms.DataFormats]::FileDrop)))
        {
            ...
            $isSupportedFile =
            $extension -ieq ".exe" -or
            $extension -ieq ".lnk" -or
            $extension -ieq ".url"
...
function Set-GaloreQuickAccessBarLocation {
    ...
    if(
        $null -eq $script:GaloreQuickAccessBar -or
        $script:GaloreQuickAccessBar.IsDisposed -or
        $LauncherForm.IsDisposed -or
        $LauncherForm.WindowState -eq [System.Windows.Forms.FormWindowState]::Minimized -or
        $LauncherForm.ClientSize.Width -le 0 -or
        $LauncherForm.ClientSize.Height -le 0
    )
    {
        return
    }

    $location = $LauncherForm.PointToScreen(
        [System.Drawing.Point]::new(
            0,
            ($LauncherForm.ClientSize.Height + 4)
        )
    )
    ...
    if(
        $null -ne $script:GaloreQuickAccessDropSurface -and
...
function Get-GaloreQuickAccessImage {
    ...
    try
    {
        if(Test-Path -LiteralPath $Path -PathType Container)
        {
            return [System.Drawing.Bitmap]::new(
                (Get-GaloreResourcePath "folder.png")
            )
        }
    ...
    {
        return [System.Drawing.Bitmap]::new(
            (Get-GaloreResourcePath "exe.png")
        )
    }
...
function Update-GaloreQuickAccessBar {
    ...
    if(
        $null -eq $script:GaloreQuickAccessBar -or
        $script:GaloreQuickAccessBar.IsDisposed
    )
    {
        return
    }
    ...
    foreach($item in @($script:GaloreQuickAccessItems))
    {
        ...
        $entry.Add_MouseMove({
            ...
            if(
                $e.Button -ne [System.Windows.Forms.MouseButtons]::Left -or
                $null -eq $sender.Tag.StartPoint -or
                $sender.Tag.DragStarted
            )
            {
                return
            }
            ...
            if(
                [Math]::Abs($e.X - $sender.Tag.StartPoint.X) -lt 4 -and
                [Math]::Abs($e.Y - $sender.Tag.StartPoint.Y) -lt 4
            )
            {
                return
            }
            ...
            $removeItem =
            @($script:GaloreQuickAccessItems | Where-Object { $_.Path -eq $sender.Tag.Path }) |
            Select-Object -First 1
        ...
        $entry.Add_MouseUp({
            ...
            if(
                $e.Button -eq [System.Windows.Forms.MouseButtons]::Left -and
                -not $sender.Tag.DragStarted
            )
            {
                try
                {
...
function Initialize-GaloreQuickAccessBar {
    ...
    $bar.Add_MouseDown({
        ...
        if($e.Button -eq [System.Windows.Forms.MouseButtons]::Right)
        {
            Remove-GaloreQuickAccessItem `
            -Path ([string]$entry.Tag.Path)
    ...
    $bar.Add_MouseMove({
        ...
        $entry = @($sender.Controls | Where-Object {
            $null -ne $_.Tag.StartPoint -and
            $_.Bounds.Contains($_.Tag.StartPoint)
        }) | Select-Object -First 1
        ...
        if(
            [Math]::Abs($e.X - $entry.Tag.StartPoint.X) -lt 4 -and
            [Math]::Abs($e.Y - $entry.Tag.StartPoint.Y) -lt 4
        )
        {
            return
        }
...
foreach($callbackName in @(
    ...
    "Save-GaloreQuickAccessItems",
    "Remove-GaloreQuickAccessItem",
    "Add-GaloreQuickAccessDroppedItems",
    "Set-GaloreQuickAccessBarLocation",
    "Update-GaloreQuickAccessBar",
    "Render-GaloreQuickAccessBar"
))
{
