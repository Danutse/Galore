...
function Clear-RAM
{
    ...
    if(
        $script:RAMCleanerPowerShell
    )
    {

        $cleanerState =
        $script:RAMCleanerPowerShell.InvocationStateInfo.State
        ...
        try
        {
            ...
            )
            {
                ...
                $script:RAMCleanerPowerShell.EndInvoke(
                    $script:RAMCleanerAsyncResult
                ) | Out-Null
        ...
        catch
        {
            ...
            -Exception $_ `
            -Context "The previous RAM-cleanup operation did not complete normally."
...
if(
    Test-Path `
    -LiteralPath $NvidiaSensorReaderPath `
    -PathType Leaf
)
{
    ...
    -Level "INFO" `
    -Component "Hardware" `
    -Message "NVIDIA sensor reader available."
...
else
{
    ...
    -Level "WARNING" `
    -Component "Hardware" `
    -Message "NVIDIA sensor reader unavailable; GPU fallback values will be used."
...
-Level "INFO" `
-Component "Hardware" `
-Message "Background hardware monitor started."

$hardwareDiagnosticLogPath =
Join-Path `
$AppRoot `
"Logs\Diagnostics.log"

$hardwareActivityLogPath =
Join-Path `
$AppRoot `
"Logs\GaloreLauncher.log"

$script:HardwareJob =
Start-Job -ArgumentList $AppRoot,$hardwareDiagnosticLogPath,$hardwareActivityLogPath -ScriptBlock {
...
while($true)
{
    ...
    try
    {
        ...
        $ram =
        ($ramUsed / $memory.TotalVisibleMemorySize) * 100
    ...
    catch
    {
        ...
        if(
            -not $loggedHardwareErrors.ContainsKey(
                $errorMessage
            )
        )
        {

            $loggedHardwareErrors[$errorMessage] =
            $true
...
function Initialize-HardwareCacheReader {
    ...
    $script:HardwareReadTimer.Interval =
    100
